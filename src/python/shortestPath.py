...
def shortestPath(graph, origen, destiny):
    ...
    #    When we find a non-existent node in the dictionary, it will be considered as infinity and the previous node will be None
    ...
    while len(queue) > 0 and len(distances) < len(graph):
        ...
        for neighbor, neighborCost in graph[currentNode].items():
            ...
            if neighborCumulativeCost < distances[neighbor][0]:

                #  Update the distance of the neighbor and the previous node to can build the path
                distances[neighbor] = (neighborCumulativeCost, currentNode)
    ...
    while destiny != origen:
        destiny = distances[destiny][1]
...
origen = 'S'
destiny = 'E'
path = shortestPath(graph, origen, destiny)
