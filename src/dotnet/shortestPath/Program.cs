...
var origen = 'A';
var destiny = 'E';
var path = ShortestPath(graph, origen, destiny);

WriteLine(string.Join(" -> ", path));


static char[] ShortestPath(Graph graph, char origin, char destiny)
{
    ...
    //   <char, (char Prev, int Distance)> -> <Node, (PreviousNode, Distance)>
    var distances = new Dictionary<char, (char Prev, int Distance)>();
    ...
    while (queue.Count > 0 && visited.Count < graph.Count)
    {
        ...
        { // We found the destiny node, so we can stop because we have the shortest path
            break;
        }
        ...
        foreach (var (neighbor, neighborCost) in graph[currentNode])
        {
            ...
            if (!distances.ContainsKey(neighbor) ||
                neighborCumulativeCost < distances[neighbor].Distance)
            {
                // Update the distance of the neighbor and the previous node to can build the path
                distances[neighbor] = (currentNode, neighborCumulativeCost);
    ...
    // Build the path from the destiny to the origin
    var path = new List<char>() { destiny };
    while (destiny != origin)
    {
        destiny = distances[destiny].Prev;
        path.Add(destiny);
    }

    // Reverse the path to get the origin -> destiny path
    return path.Reverse<char>().ToArray();
}
